...
def gost_hash(string: str, isHex=False):
    PI = (252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77, 233, 119, 240, 219, 147, 46, 153,
          186, 23, 54, 241, 187, 20, 205, 95, 193, 249, 24, 101, 90, 226, 92, 239, 33, 129, 28, 60, 66, 139, 1, 142, 79,
          5, 132, 2, 174, 227, 106, 143, 160, 6, 11, 237, 152, 127, 212, 211, 31, 235, 52, 44, 81, 234, 200, 72, 171,
    ...
    if not isHex:
        ...
        s_bits = [int(i) for i in bin(int(string, 16))[2:].zfill(len(string) * 4)]

    hash_bits = H(s_bits)

    return ''.join(
        hex(i)[2:] for i in [
            int(''.join(map(str, hash_bits[i:i + 4])), 2) for i in range(0, len(hash_bits), 4)
        ]
    )
